...
st.info("Verified Historical Audit: Campaigns + Search Terms + Sales Export")

st.sidebar.header("Upload Files")
ad_file = st.sidebar.file_uploader("1. Ad Overview (Campaign Report)", type=["csv", "xlsx"])
term_file = st.sidebar.file_uploader("2. Search Term / Target Report", type=["csv", "xlsx"])
sales_file = st.sidebar.file_uploader("3. Sales Export (Overall Sales)", type=["csv", "xlsx"])

if ad_file and term_file and sales_file:
    def load_df(file):
        df = pd.read_csv(file) if file.name.endswith('.csv') else pd.read_excel(file)
        df.columns = [str(c).strip() for c in df.columns]
        for col in df.columns:
            if not any(x in col.lower() for x in ['name', 'title', 'code', 'sku', 'target', 'value']):
                df[col] = df[col].apply(clean_numeric)
        return df

    # 1. Load and Brand-Map Data
    ads_df = load_df(ad_file)
    terms_df = load_df(term_file)
    overall_df = load_df(sales_file)

    ads_df['Brand'] = ads_df['Campaign Name'].apply(get_brand_robust)
    terms_df['Brand'] = terms_df['Campaign Name'].apply(get_brand_robust)
    ...
    }).rename(columns={'Views': 'Impressions', 'Spends': 'Ad Spend', 'Revenue': 'Ad Sales'}).reset_index()
    ...
    for i, brand_name in enumerate(sorted(BRAND_MAP.values())):
        with tabs[i+1]:
            ...
            b_camp = ads_df[ads_df['Brand'] == brand_name][['Campaign Name', 'Views', 'Clicks', 'Spends', 'Revenue', 'ROAS']]
            st.dataframe(b_camp.sort_values(by='Revenue', ascending=False), hide_index=True, use_container_width=True)
            
            st.divider()
            # 🔍 Search Term Overview
            st.subheader(f"🔍 {brand_name} Search Term / Target Performance")
            # Noon Target reports use 'Target Value' for keywords
            search_col = 'Target Value' if 'Target Value' in terms_df.columns else 'Search Term'
            b_terms = terms_df[terms_df['Brand'] == brand_name][['Campaign Name', search_col, 'Views', 'Clicks', 'Spends', 'Revenue', 'ROAS']]
            st.dataframe(b_terms.sort_values(by='Spends', ascending=False), hide_index=True, use_container_width=True)
    ...
    st.info("Please upload Ad Overview, Search Term Report, and Sales Export to generate the audit.")
